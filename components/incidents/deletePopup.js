...
import { deleteIncidentsAPI } from "../../redux/APIS/API";
import { useRouter } from "next/router";
import { RiDeleteBin6Line } from "react-icons/ri";

function DeletePopup(props) {
  const incidensId = useSelector(
    (state) => state?.incidentsDetails?.incidentsDetails?.ID
  );

  // console.log(incidensId, "incidensId delete");
  ...
  async function deleteUser() {
    try {
      const respon = await deleteIncidentsAPI({
        incident_ids: [incidensId],
      });
