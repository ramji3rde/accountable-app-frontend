...
import { deleteSingleSecureNotesAPI } from "../../../redux/APIS/API";
...
function DeletePopup(props) {
  const itemID = useSelector(
    (state) => state?.secureNotesDetails?.secureNotesDetails?.data?.id
  );
  ...
  async function deleteUser() {
    try {
      const respon = await deleteSingleSecureNotesAPI({
        note_ids: [itemID],
